...
emb_name = f"embedding_test_{os.path.basename(sys.argv[2]).split('.')[0]}_{os.path.basename(sys.argv[3])}_{os.path.basename(sys.argv[4])}.npy"
if os.path.isfile(emb_name):
    logger.info(f"load embedding from {emb_name}")
    embeddings_test = np.load(emb_name)
else:
    embeddings_test = feature_extractor.extract_feature(df=df_test)
    np.save(emb_name, embeddings_test)
...
if len(sys.argv) == 6:  # path 指定
    os.makedirs(sys.argv[5], exist_ok=True)
    df_test.to_parquet(f"{sys.argv[5]}/{os.path.basename(sys.argv[1])}")    
else:
