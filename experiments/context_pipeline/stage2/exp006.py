...
class DataCollatorForMultipleChoice:
    ...
    def __call__(self, features):
        ...
        batch = self.tokenizer.pad(
            ...
            max_length=len(features[0]["input_ids"][0]),
...
def preprocess_df(df, config, num_context):
    df["context"] = ""
...
def main(config: BertConfig):
    try:
        ...
        df_train = preprocess_df(df_train, config, num_context=config.num_context)
        ...
        df_valid = preprocess_df(df_valid, config, num_context=6)
        
        df_test = preprocess_df(df_test, config, num_context=6)
        ...
        def preprocess(example, max_length):
            try:
                if config.num_context == 0:
        ...
        func = partial(preprocess, max_length=768)
        test_dataset = Dataset.from_pandas(df_test)
        tokenized_test_dataset = test_dataset.map(func, remove_columns=["prompt", "context", "A", "B", "C", "D", "E"])
        ...
        func = partial(preprocess, max_length=config.max_length)
        tokenized_train_dataset = train_dataset.map(func, remove_columns=["prompt", "context", "A", "B", "C", "D", "E"])

        func = partial(preprocess, max_length=768)
        valid_dataset = Dataset.from_pandas(df_valid)
        tokenized_valid_dataset = valid_dataset.map(func, remove_columns=["prompt", "context", "A", "B", "C", "D", "E"])
...
if __name__ == "__main__":
    ...
    config = BertConfig(
        ...
        warmup_ratio=0.01,
        max_grad_norm=10,
        experiment_name=f"gtesmall_l90s3_maxlen256_bs2_lr1e-6_4epochs_msdeberta",
    ...
    for max_length, ncontexts in zip([192, 256, 384], [2, 3, 4]):
        config = BertConfig(
            debug=False,
            epochs=4,
            batch_size=2,
            ...
            warmup_ratio=0.01,
            max_grad_norm=10,
            experiment_name=f"gtesmall_l90s3_maxlen{max_length}_ncontexts{ncontexts}_lr1e-6_3epochs",
            dataset_dir=f"output/context_pipeline/stage1/exp009.py/20231005133048_gte-small_wikiall_without_sep_targetprompt_and_choice_without_sep_token_length90_stride_sentence3_drop_categoryTrue_all",  # ここに作ったデータ入れる
            freeze_layers=0,
            max_length=max_length,
            num_context=ncontexts,
            data_source_without=[
                "additional_data/MMLU/test.csv", "additional_data/MMLU/train.csv",
                "20230924083304_gpt5",
            ]
