    ...
    def __getitem__(self, idx):
        ...
        for choice in ["A", "B", "C", "D", "E"]:
            ...
            for i in range(self.cfg.num_context):
                context += row[f"searched_wiki_id_{i}"] + "\n"

            text = row["prompt"] + " [SEP] " + row[choice] + " [SEP] " + context
            text = self.tokenizer(
                text,
                padding="max_length",
...
def main(config: BertConfig):
    try:
        ...
        output_dir = f"output/exp001/{dt.now().strftime('%Y%m%d%H%M%S')}_{config.experiment_name}"
        ...
        for fold, (train_index, valid_index) in enumerate(kfold.split(df)):
            ...
            for epoch in range(config.epochs):
                ...
                df_valid_pred, _ = valid_one_epoch(model, valid_loader, device, fold, epoch, name="valid")
...
if __name__ == "__main__":
    ...
    for freeze_layers in [18, 21]:
        for freeze_embeddings in [False, True]:
            config = BertConfig(
                debug=False,
                experiment_name=f"{freeze_layers}layers_freeze_and_freeze_embeddings_{freeze_embeddings}",
