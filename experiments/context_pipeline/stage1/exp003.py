...
def main(config: Config):
    ...
    try:
        ...
        text_original_col = "original_text" if "original_text" in df_wiki.columns else "text"
        df_wiki["text_original_fixed"] = feature_extractor._preprocess_texts(df=df_wiki, text_name=text_original_col)
        wiki_dict = dict(zip(df_wiki["id"].values, df_wiki["text_original_fixed"].values))
        ...
        searched_ids = search(index=index, embeddings=embeddings_target, ids=df_wiki["id"].values, k=200)
...
if __name__ == "__main__":    
    ...
    #         exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_M{M}_bits{bits}only_a"
    #         config = Config(
    ...
    # for nlists in [16, 32, 64, 128, 256]:
    #     for bits in [8]:
    #         # exp_name = f"{model_name}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_only_a_text"
    #         exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_nlists{nlists}_bits{bits}only_a"
    #         config = Config(
    #             experiment_name=exp_name,
    #             wiki_file_name="data/wikipedia/a.parquet",
    #             # wiki_file_name="data/wikipedia/all.parquet",
    ...
    #         # wiki_file_name="data/wikipedia/all.parquet",
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_all.npy",
    #         # extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_all.npy",
    #         model_name=model_name,
    #         max_length=max_length,
    #         wiki_text_preprocess=wiki_text_preprocess,
    ...
    #             # wiki_file_name="data/wikipedia/all.parquet",
    #             extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_only_a.npy",
    #             # extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_all.npy",
    #             model_name=model_name,
    #             max_length=max_length,
    #             batch_size=128,
    ...
    for wiki_file_name in [
        ...
        config = Config(
            ...
            wiki_text_preprocess=wiki_text_preprocess,
            target_text_preprocess=target_text_preprocess,
            target_texts_name=f"data/gpt_generated_data/20230906_merge_dataset.csv",
            feature_extractor="sentence_transformer",
        )
        main(config)