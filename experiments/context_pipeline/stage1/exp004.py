...
def main(config: Config):
    ...
    try:
        ...
        if ".parquet" in config.wiki_file_name:
            df_wiki = pd.read_parquet(config.wiki_file_name)
        if ".pickle" in config.wiki_file_name:
            df_wiki = pd.read_pickle(config.wiki_file_name)
        if ".feather" in config.wiki_file_name:
            df_wiki = pd.read_feather(config.wiki_file_name)
        ...
        df_wiki["text_id"] = np.arange(len(df_wiki))
        wiki_dict = dict(zip(df_wiki["text_id"].values, df_wiki["texts_concatenate"].values))
        
        wiki_ids = df_wiki["id"].values
        text_ids = df_wiki["text_id"].values
        ...
        df_test["searched_text_ids"] = search(index=index, embeddings=embeddings_test, ids=df_wiki["text_id"].values, k=30)
...
if __name__ == "__main__":        
    ...
    #     exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_{wiki_file_name}_a"
    #     config = Config(
    #         experiment_name=exp_name,
    #         wiki_file_name=f"data/wikipedia/sep/{wiki_file_name}/a.parquet",
    #         # wiki_file_name="data/wikipedia/all.parquet",
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_only_a_exp004.npy",
    ...
    #         # wiki_file_name="data/wikipedia/all.parquet",
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_all_exp004.npy",
    #         # extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_all.npy",
    #         model_name=model_name,
    #         max_length=max_length,
    #         batch_size=128,
    #         wiki_text_preprocess=wiki_text_preprocess,
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/all_12_with_context2.csv",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
    ...
    #     config = Config(
    #         experiment_name=exp_name,
    #         wiki_file_name=f"data/wikipedia/sep2/{wiki_file_name}/a.parquet",
    #         # wiki_file_name="data/wikipedia/all.parquet",
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_only_a_exp004_sep2.npy",
    #         # extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_all.npy",
    #         model_name=model_name,
    #         max_length=max_length,
    #         batch_size=128,
    ...
    for wiki_file_name in [
        ...
        config = Config(
            experiment_name=exp_name,
            wiki_file_name=f"data/wikipedia/sep2/{wiki_file_name}/all.feather",
            # wiki_file_name="data/wikipedia/all.parquet",
            extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_all_exp004_sep2.npy",
            # extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_all.npy",
            model_name=model_name,
            max_length=max_length,
            batch_size=128,
