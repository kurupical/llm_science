...
class FeatureExtractor:
    ...
    def _extract_feature_bert(
        ...
        ret = []
        for model_name in self.config.model_names:
            model = pipeline("feature-extraction", model=model_name, torch_dtype=torch.float16)
    ...
    def extract_feature(
        ...
        if file != "":  # wiki の場合は file != "".
            embeddings = np.concatenate([
                np.load(f"{f}_{os.path.basename(file).split('.')[0]}.npy") for f in self.config.extract_file_names
            ], axis=1)
            self.logger.info(f"embedding の shape: {embeddings.shape}")
            return embeddings
...
if __name__ == "__main__":        
    ...
    extract_file_names = [
        f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_only_a_exp009"
        for model_name in model_names
    ]
    exp_name = f"ensemble_gte_wiki{wiki_text_preprocess}_target{target_text_preprocess}_{wiki_file_name}_only_a"
    config = Config(
        experiment_name=exp_name,
        wiki_file_names=glob.glob(f"data/wikipedia_fixed/sep_a/{wiki_file_name}/chunk1.parquet"),
