...
class Config:
    ...
    faiss_index_parameter: dict = dataclasses.field(
        default_factory=lambda: {"nlists": 128, "M": 16, "nbits": 8}
    )
...
class FeatureExtractor:
    ...
    def extract_feature(
        ...
        if self.config.feature_extractor == "sentence_transformer":
            embeddings = self._extract_feature_sentence_transformer(texts)
        if self.config.feature_extractor == "bert":
...
class FeatureExtactorFromPrompt(FeatureExtractor):
    ...
    def _preprocess_texts(
        ...
        if self.text_preprocess == "prompt_and_choice":
            return df["prompt"] + " [SEP] " + df["A"] + " [SEP] " + df["B"] + " [SEP] " + df["C"] + " [SEP] " + df["D"] + " [SEP] " + df["E"]
...
def main(config: Config):
    ...
    try:
        ...
        df_target.to_csv(f"{output_dir}/searched_index_train.csv", index=False)
...
if __name__ == "__main__":
    ...
    # for wiki_text_preprocess in [
    #     "all",
    #     "title_and_abstract",
    # ]:    
    #     for target_text_preprocess in [
    #         "prompt_and_choice",
    #         "only_prompt",
    #     ]:
    #         for model_name in [
    ...
    #         ]:
    #             exp_name = f"{model_name}_wiki{wiki_text_preprocess}_target{target_text_preprocess}"
    #             config = Config(
    #                 experiment_name=exp_name,
    #                 extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_only_a.npy",
    ...
    # for nlists in [128, 256, 512, 1024]:
    #     for M in [16, 32, 64, 128]:
    #         config = Config(
    #             experiment_name=f"all-MiniLM-L6-v2_all_{nlists}_{M}",
    #             extract_file_name=f"output/embeddings/all-MiniLM-L6-v2_all_only_a.npy",
    ...
    #             # "sentence-transformers/msmarco-roberta-base-v3",
    #         ]:
    #             exp_name = f"{model_name}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_all_text"
    #             config = Config(
    #                 experiment_name=exp_name,
    ...
    #         config = Config(
    #             experiment_name=f"all-MiniLM-L6-v2_all_{nlists}_{M}",
    #             extract_file_name=f"output/embeddings/all-MiniLM-L6-v2_all_all.npy",
    #             model_name="sentence-transformers/all-MiniLM-L6-v2",
    #             wiki_text_preprocess="all",
    #             feature_extractor="sentence_transformer",
    #             faiss_index_parameter={
    ...
    for wiki_text_preprocess in [
        ...
        for target_text_preprocess in [
            ...
            for model_name in [
                ...
                exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_all_text"
                config = Config(
                    experiment_name=exp_name,
        ...
        for target_text_preprocess in [
            ...
            for model_name in [
                ...
                config = Config(
                    ...
                    wiki_text_preprocess=wiki_text_preprocess,
                    target_text_preprocess=target_text_preprocess,
                    target_texts_name="data/gpt_generated_data/20230903025102.csv",
                    feature_extractor="sentence_transformer",
                )
                main(config)