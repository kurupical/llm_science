...
class Config:
    ...
    faiss_index_parameter: dict = dataclasses.field(
        default_factory=lambda: {"nlists": 1, "M": 64, "nbits": 8}
    )
...
def main(config: Config):
    ...
    try:
        ...
        if ".parquet" in config.target_texts_name:
            df_target = pd.read_parquet(config.target_texts_name).drop_duplicates()
        if ".csv" in config.target_texts_name:
        ...
        for file in config.wiki_file_names:
            ...
            texts_concatenate = feature_extractor._preprocess_texts(df_wiki).values.tolist()
            ...
            searched_ids, _ = search(index=index, embeddings=embeddings_target, ids=wiki_ids, k=50)
            ...
            searched_text_ids, similarity = search(index=index, embeddings=embeddings_target, ids=text_ids, k=50)
            ...
            searched_ids, _ = search(index=index, embeddings=embeddings_test, ids=wiki_ids, k=50)
            df_test["searched_ids"] = [x + searched_ids[i] for i, x in enumerate(df_test["searched_ids"].values)]
            searched_text_ids, similarity = search(index=index, embeddings=embeddings_test, ids=text_ids, k=50)
            ...
            del index, wiki_dict, df_wiki; gc.collect()
            libc.malloc_trim(0)
        logger.info("後処理")
        ...
        df_target["similarity_rank"] = [np.argsort(x)[:50] for x in df_target["similarity"].values]
        df_test["similarity_rank"] = [np.argsort(x)[:50] for x in df_test["similarity"].values]
        ...
        df_target[df_target["dataset"] == "train"].reset_index(drop=True).to_feather(f"{output_dir}/train.feather")
        df_target[df_target["dataset"] == "valid"].reset_index(drop=True).to_feather(f"{output_dir}/valid.feather")
        df_test.reset_index(drop=True).to_feather(f"{output_dir}/test.feather")
...
if __name__ == "__main__":        
    ...
    #     config = Config(
    #         experiment_name=exp_name,
    #         wiki_file_names=glob.glob(f"data/wikipedia_fixed/sep_chunk_wiki270k/{wiki_file_name}/*.parquet"),
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_all_exp009_wiki270k",
    #         model_name=model_name,
    #         max_length=max_length,
    #         batch_size=128,
    #         wiki_text_preprocess=wiki_text_preprocess,
    ...
    #         target_texts_name=f"data/gpt_generated_data/only_a_250text_3.csv",
    #         feature_extractor="sentence_transformer",
    #         faiss_index_parameter={"nlists": 1, "M": 64, "nbits": 8}
    #     )
    #     main(config)
    ...
    #         wiki_text_preprocess=wiki_text_preprocess,
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/20230926085253_gpt6.parquet",
    #         pretrain_index_dir="output/context_pipeline/stage1/exp009.py/20230922162941_gte-base_wikiall_without_sep_targetprompt_and_choice_without_sep_token_length120_stride_sentence4_drop_categoryTrue_all",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
    ...
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/20230925_concatenate.csv",
    #         pretrain_index_dir="output/context_pipeline/stage1/exp009.py/20230928121714_gte-base_wikiall_without_sep_targetprompt_and_choice_without_sep_token_length120_stride_sentence4_drop_categoryTrue_all",
    #         feature_extractor="sentence_transformer",
    #         faiss_index_parameter={"nlists": 1, "M": 64, "nbits": 8}
    #     )
    #     main(config)
    ...
    # for wiki_file_name in [
    #     "token_length120_stride_sentence4_drop_categoryTrue_chunked",
    # ]:
    #     exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_{wiki_file_name}_all"
    #     config = Config(
    #         experiment_name=exp_name,
    #         wiki_file_names=glob.glob(f"data/wikipedia_fixed/sep_chunk/{wiki_file_name}/*.parquet"),
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_token_length120_stride_sentence4_drop_categoryTrue_all_exp009",
    #         model_name=model_name,
    #         max_length=max_length,
    #         batch_size=128,
    #         wiki_text_preprocess=wiki_text_preprocess,
    ...
    # for wiki_file_name in [
    #     # "token_length120_stride_sentence4_drop_categoryTrue",
    #     # "token_length120_stride_sentence6_drop_categoryTrue",
    #     # "token_length120_stride_sentence8_drop_categoryTrue",
    #     # "token_length120_stride_sentence10_drop_categoryTrue",
    #     # "token_length150_stride_sentence10_drop_categoryTrue",
    #     # "token_length150_stride_sentence12_drop_categoryTrue",
    #     # "token_length90_stride_sentence4_drop_categoryTrue",
    #     # "token_length90_stride_sentence6_drop_categoryTrue",
    #     "token_length150_stride_sentence6_drop_categoryTrue",
    # ]:
    #     exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_{wiki_file_name}_only_a"
    #     config = Config(
    #         experiment_name=exp_name,
    #         wiki_file_names=glob.glob(f"data/wikipedia_fixed/sep_a2/{wiki_file_name}/chunk1.parquet"),
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_only_a_exp009_2",
    #         model_name=model_name,
    #         max_length=max_length,
    #         batch_size=512,
    #         wiki_text_preprocess=wiki_text_preprocess,
    ...
    #     config = Config(
    #         experiment_name=exp_name,
    #         wiki_file_names=glob.glob(f"data/wikipedia_fixed/sep_chunk2/{wiki_file_name}/*.parquet"),
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_all_exp009",
    #         model_name=model_name,
    #         max_length=max_length,
    #         batch_size=512,
    ...
    # max_length = 144
    # # exp_name = f"{model_name}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_only_a_text"
    # wiki_file_name = "token_length90_stride_sentence4_drop_categoryTrue"
    # for model_name in [
    ...
    #         experiment_name=exp_name,
    #         wiki_file_names=glob.glob(f"data/wikipedia_fixed/sep_chunk2/{wiki_file_name}/*.parquet"),
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_all_exp009_chunk2",
    #         model_name=model_name,
    #         max_length=max_length,
    #         batch_size=512,
    #         wiki_text_preprocess=wiki_text_preprocess,
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/20230922_concatenate.csv",
    #         pretrain_index_dir="index/l90_s3_new",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
    ...
    # for wiki_file_name in [
    #     "token_length60_stride_sentence2_drop_categoryTrue",
    # ]:
    #     exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_{wiki_file_name}_all"
    #     config = Config(
    #         experiment_name=exp_name,
    ...
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/20230922_concatenate.csv",
    #         pretrain_index_dir="index/l120_s4_old",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
    ...
    #         wiki_text_preprocess=wiki_text_preprocess,
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/eval300_gpt4.csv",
    #         pretrain_index_dir="output/context_pipeline/stage1/exp009.py/20230922162941_gte-base_wikiall_without_sep_targetprompt_and_choice_without_sep_token_length120_stride_sentence4_drop_categoryTrue_all",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
    ...
    #         wiki_text_preprocess=wiki_text_preprocess,
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/20231005_concatenate.csv",
    #         pretrain_index_dir="index/l90_s3_new",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
    ...
    for wiki_file_name in [
        "token_length90_stride_sentence2_drop_categoryTrue",
    ]:
        exp_name = f"{os.path.basename(model_name)}_wiki{wiki_text_preprocess}_target{target_text_preprocess}_{wiki_file_name}_all"
        config = Config(
            experiment_name=exp_name,
            ...
            wiki_text_preprocess=wiki_text_preprocess,
            target_text_preprocess=target_text_preprocess,
            target_texts_name=f"data/gpt_generated_data/20231005_concatenate_val_only.csv",
            pretrain_index_dir="index/l90_s2_e5_base",
            feature_extractor="sentence_transformer",
        )
        main(config)

    # # for debug submission
    # wiki_text_preprocess = "all_without_sep"
    # target_text_preprocess = "prompt_and_choice_without_sep"
    # model_name = "thenlper/gte-base"
    # max_length = 256
    ...
    #         experiment_name=exp_name,
    #         wiki_file_names=[
    #             f"data/wikipedia_fixed/sep_chunk/{wiki_file_name}/chunk24.parquet",
    #             f"data/wikipedia_fixed/sep_chunk/{wiki_file_name}/chunk8.parquet",
    #             f"data/wikipedia_fixed/sep_chunk/{wiki_file_name}/chunk22.parquet"
    #         ],
    #         extract_file_name=f"output/embeddings/{os.path.basename(model_name)}_{wiki_text_preprocess}_{max_length}_{wiki_file_name}_all_exp009",
    #         model_name=model_name,
    #         max_length=max_length,
    ...
    #         wiki_text_preprocess=wiki_text_preprocess,
    #         target_text_preprocess=target_text_preprocess,
    #         pretrain_index_dir="index/l90_s3_gte_base_old",
    #         target_texts_name=f"data/gpt_generated_data/20231005_concatenate_val_only.csv",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
    ...
    #         target_text_preprocess=target_text_preprocess,
    #         target_texts_name=f"data/gpt_generated_data/20231005_concatenate_val_only.csv",
    #         pretrain_index_dir="index/l90_s4_gte_large",
    #         feature_extractor="sentence_transformer",
    #     )
    #     main(config)
